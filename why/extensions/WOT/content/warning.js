...
//        console.log("make_categories_block", categories);
	...
	add: function(data, type_obj, show_wtip) {

		try {
			var reason = type_obj.reason;
			var wls = (type_obj.type == 2 ? "" : "_light");
            ...
            if (is_blacklisted) { // If warning should show Blacklisted status
				...
				if(is_warning_light) {
					replaces.push({from: "DESC", to: wot.i18n("warnings", "reputation" + wls)});
				}
				else {
			...
			wrapper.innerHTML = wot.utils.processhtml(this.make_warning(categories, blacklists, {is_warning_light:is_warning_light}), replaces);
	...
	onload: function()
	{
		...
		wot.bind("message:warnings:show", function(port, data) {
			...
			wot.warning.add(data.data, data.type, data.show_wtip);
